...
Программы `СхемАтор` и `ПлатФорм` являются портативными и не требуют установки. Скачайте этот репозиторий в виде ZIP-архива и распакуйте его. Войдите в папку `BIN` и запустите исполняемый файл `СхемАтор.ехе`, откройте краткое руководство через меню `Помощь -> PDF-документ` и начните работать
...
|2.4.22(текущая)|Добавление фона детали через меню «Файл -> Картинка полилинии» (схемАтор)|19.03.2025|
