...
**_Совет: Рекомендуется полностью исключить кириллицу и спецсимволы из пути к папке проектов Схематора, т.к. иначе OpenSCAD может не открывать 3D-модель._**
...
Программы `СхемАтор` и `ПлатФорм` являются портативными и не требуют установки. Скачайте этот репозиторий в виде [ZIP-архива](https://github.com/niconson/PlatForm/archive/refs/heads/main.zip)(новейшая версия отладки, рекомендуется) или [релиз](https://sourceforge.net/projects/niconson/files/2.4.26/skhemAtor%20_%20platForm%20source%20code.zip/download) и распакуйте его. Войдите в папку `BIN` и запустите исполняемый файл `СхемАтор.ехе`. Руководство пользователя доступно через меню `Помощь -> PDF-документ`
...
| 2.4.26 | Кнопки перехода редакторов на верхней панели. Улучшение тестирования трасс | 29.06.2025 |
| 2.4.28 | Поиск футпринтов по расстоянию между пинами, альтернативные функции пина на схеме, область запрета в футпринте, требование проверки DRC перед генерацией герберов, панелизация с репперными знаками на стадии формирования герберов, фрезерный контур для панелизации плат со сложным контуром, симметризация мультизаготовки для оптимизации производства, генерация CSV-report для панелизированных плат, DXF для лазерного гравёра, G-CODE для фрезерного станка, интерактивный вид печатной в схематоре (скрипт), ноуды на концах полилинии(F9), возм.сепарированной печати ПДФ в один файл, проекции слоя TOP_COPPER или BOTTOM_COPPER и ещё 11 новых видовых вариантов в OpenSCAD, оптимизация, мелкие доработки | 19.03.2026 |
| 2.4.29(текущая) | Автоматическое выравнивание при перетаскивании группы в схемном редакторе по узлам проводов и компонентов на схеме. Шаг выбранной сетки в данном случае является областью действия магнита. В Схематоре добавлена новая функция - теперь можно в текстовом файле хранить пользовательские поля, например артикул компонента, и затем выводить его в БОМ-файле. В текстовом файле компонента надо с новой строки создать заголовок поля в квадратных скобках например так: [Артикул] а строкой ниже записать само значение  | ХХ.ХХ.2026 |
